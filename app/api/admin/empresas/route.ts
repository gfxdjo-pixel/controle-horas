...
// LISTAR TODAS AS EMPRESAS (Apenas para você)
export async function GET() {
  const session = await getServerSession(authOptions);
  if (session?.user?.email !== 'gfxdjo@gmail.com') return NextResponse.json({ error: 'Negado' }, { status: 403 });

  try {
    // Buscamos as empresas e o e-mail do admin vinculado a cada uma
...
// CRIAR EMPRESA (Já temos, mantemos a lógica)
...
export async function DELETE(req: Request) {
  ...
  try {
    // Primeiro removemos os usuários vinculados (por segurança do banco)
    await sql`DELETE FROM perfis_usuarios WHERE empresa_id = ${id}`;
    // Depois deletamos a empresa
